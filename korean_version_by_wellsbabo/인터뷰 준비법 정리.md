...
- 기술 역량: 논의된 해결책을 코딩으로 옮기는 능력. 큰 문제 없이 정확하고 깨끗한 코드를 작성하고, 언어의 주요 구문을 잘 이해해야 합니다.
...
아래에 코딩 인터뷰 준비 3개월 스터디 플랜을 정리해놓았습니다. 아래의 내용을 통해 공부해야할 정확한 주제와 풀어봐야할 구체적인 연습 문제를 알 수 있습니다.
...
> "multiplesOfThree"와 같은 변수 이름을 사용해 코드를 쉽게 설명할 수 있도록 하세요.
...
문제가 크면, 고수준의 함수를 먼저 작성한 후, 더 작은 함수들로 나누어 각각 따로 해결하세요. 이렇게 하면 세부 사항에 압도되지 않고 구조적으로 사고할 수 있습니다.
...
문제를 분석할 때 어떤 데이터 구조와 알고리즘이 적합한지 파악하는 능력을 기르기 위해, 이러한 기법을 연습하는 것이 중요합니다.
...
답을 이미 얻었으면 즉시 반환하세요. 예를 들어, "대소문자 구분 없이 문자열 배열에 특정 문자열이 있는지 확인하는 문제"를 살펴보겠습니다.
...
이것은 미세한 최적화(micro-optimization)로, 대부분의 경우 문자열이 비어 있지는 않겠지만, 불필요한 계산을 피할 수 있는 예를 보여주기 위한 예시입니다. 이는 코드에서 필요한 경우에만 계산을 수행하는 Lazy Evaluation을 적용하는 예시입니다. 최대한 조건문을 통해 계산이 필요한 상황을 최소화한 이후에 계산을 수행하게 하는 방법입니다.
...
현재의 솔루션이 새로운 데이터 구조를 생성해 중간 처리나 캐싱을 수행하는 경우, 메모리 공간이 할당됩니다. 이는 그만큼 메모리 공간을 더 사용하게 되는 것입니다. 이를 피하는 하나의 방법은 원본 입력 배열의 값을 덮어써서 새 공간을 할당하지 않고 코드를 작성하는 것입니다. 그러나 **코드를 더 진행하는 부분에서 입력 데이터를 다시 사용해야 한다면, 데이터를 되돌릴 수 없는 방식으로 파괴하지 않도록 주의**해야 합니다.

> ❗주의 : 일반적으로 소프트웨어 엔지니어링에서 입력 데이터를 변경하는 것은 코드의 가독성과 유지보수성을 떨어뜨리기 때문에 권장되지 않습니다. 따라서 원본 데이터를 변경하는 것은 주로 코딩 인터뷰에서만 사용해야 합니다.

> 예시1) [Dutch National Flag](https://leetcode.com/problems/sort-colors/description/) 문제는 O(n) 시간과 O(n) 공간으로 새 배열을 생성하고, 정렬된 값을 채워넣는 방식으로 쉽게 해결할 수 있습니다. 하지만, 면접관은 공간 최적화 도전 과제로 O(n) 시간과 O(1) 공간 복잡도를 요구할 수 있습니다. 이 경우 입력 배열을 제자리에서 정렬하는 방식으로 해결해야 합니다.

> 예시2) [First Missing Positive](https://leetcode.com/problems/first-missing-positive/description/) 문제에서는 원본 배열을 해시 테이블처럼 사용해 중간 데이터를 저장할 수 있습니다. 첫 번째 for 루프 이후 배열의 모든 값이 양수이면, 해당 인덱스의 값을 음수로 만들어 숫자가 존재함을 표시할 수 있습니다. 예를 들어, 숫자 4가 있음을 나타내기 위해 nums[4] 값을 음수로 바꿀 수 있습니다.

---

##### 2. 자료 구조 변경하기

자료 구조는 코딩 인터뷰에서 매우 중요합니다. 올바른 자료 구조를 선택하는 것은 문제 해결에 필수적이며, 그에 따라 인터뷰 성과가 좌우될 수 있습니다. 문제에 가장 적합한 자료 구조를 사용하고 있나요?

> 예시) 문자열 리스트가 주어지고, 특정 접두사로 시작하는 문자열의 개수를 찾는 문제를 해결해야 한다고 가정해 보세요. 이때 문자열을 효율적으로 저장할 수 있는 방법은 무엇일까요? **트라이(Trie)**는 문자열을 저장하는 데 매우 효율적이며, 특정 접두사로 시작하는 문자열의 개수를 빠르게 계산할 수 있는 트리 형태의 자료 구조입니다.

공간 복잡도 최적화는 코딩 인터뷰에서 추가적인 도전 과제가 될 수 있으며, 올바른 자료 구조 선택이나 입력 데이터를 제자리에서 처리하는 방식으로 문제를 해결할 수 있습니다.
